...
Aggregators (Adzuna) re-list and go stale. Pulling straight from each employer's
ATS is current and truthful. Supported providers: Greenhouse, Lever, Ashby,
SmartRecruiters, Recruitee, Workday.

You just need each company's ATS "slug". Don't guess — run the discovery tool
(public endpoints, no keys, run locally with internet):

    python fetcher/discover_ats.py

It probes every provider for each company in DISCOVERY_CANDIDATES and prints
copy-paste-ready config, e.g.:

    LEVER = [
        ("Razorpay", "razorpay"),
    ]

Paste those blocks into fetcher/companies.py (GREENHOUSE / LEVER / ASHBY /
SMARTRECRUITERS / RECRUITEE), commit, and the next fetch pulls those roles tagged
"Career site". Add more companies to DISCOVERY_CANDIDATES anytime.

Workday (many big firms) can't be slug-probed — grab tenant/pod/site from the
careers page via dev-tools -> Network (URL: <tenant>.<pod>.myworkdayjobs.com/.../<site>)
and add to WORKDAY as ("Label","tenant","pod","site"). Truly closed ATS
(SuccessFactors/Taleo/Avature at some consulting firms) stay on the Adzuna fallback.
