...
import filters as F
from sources import (fetch_adzuna, fetch_greenhouse, fetch_lever, fetch_ashby,
                     fetch_smartrecruiters, fetch_recruitee, fetch_workday)
...
def run():
    ...
    # 2. reliable career-site (ATS) feeds — source of truth
    raw += fetch_greenhouse(C.GREENHOUSE)
    raw += fetch_lever(C.LEVER)
    raw += fetch_ashby(C.ASHBY)
    raw += fetch_smartrecruiters(C.SMARTRECRUITERS)
    raw += fetch_recruitee(C.RECRUITEE)
